...
(* This does not work this way in the general case *)
Lemma internal_step_to_front :
  forall n n' ι a, n -[a | ι]ₙ-> n' -> forall n'' ι', 
  n' -[AInternal | ι']ₙ-> n'' ->
  ι' <> ι
->
  exists n''', n' -[AInternal | ι']ₙ-> n''' /\ n''' -[a | ι]ₙ-> n''.
Proof.
...
->
  (exists n3 n4, a = AInternal /\ n -->* n3 /\ n3 -[a |ι]ₙ-> n4 /\ n4 -->* n') \/ 
  (exists n''', n' -[a | ι]ₙ-> n''').
Proof.
  intros n n' IH. destruct IH as [l [k [All Steps]]].
  dependent induction Steps; intros.
  * right. eexists. eauto.
  * inversion All; subst. simpl in H3. subst.
    destruct (Nat.eq_dec ι ι0); subst.
    - eapply internal_det in H0 as H0'. 2: exact H.
      destruct H0' as [[eq1 eq2] | [t H0']]; subst.
      + left. exists n, n'. split; auto.
        split. 2: split. apply internals_refl.
        auto.
        exists l, k; split; auto.
      + destruct H0' as [eq1 [eq2 [newn [D eq3]]]]. subst.
        eapply IHSteps in H4 as H4'. 2: exact D.
        destruct H4' as [[n3 [n4 [eq _]]]| [n3 D2]]. congruence.
        right. eexists; eauto.
    - eapply step_chain in H0. 2: exact H. 2-3: auto.
      destruct H0. eapply IHSteps in H0 as H0'; auto.
      destruct H0' as [[n3 [n4 [eq1 [D1 [D2 D3]]]]] | [t H0']]; subst.
      + left. exists n3, n4. split; auto.
        split. 2: split. all: auto.
        eapply internals_trans. 2: exact D1.
        exists [(AInternal, ι)], 1. split.
        repeat constructor.
        eapply ntrans. exact H. apply nrefl.
      + right. now exists t.
Qed.

Theorem delay_internal_diff :
  forall n n' ι,   n -[AInternal | ι]ₙ-> n' ->
  forall n'' ι' a, ι <> ι' -> n' -[a | ι']ₙ-> n''
->
  exists n3, n -[a | ι']ₙ-> n3 /\ n3 -[AInternal | ι]ₙ-> n''.
Proof.
...
Proof.
  ...
  * eapply concurrent_determinism in H0. 2: exact H1. subst. apply internals_refl.
  * inversion All; simpl in H5. subst.
    destruct (Nat.eq_dec ι0 ι).
    - subst.
      eapply internal_det in H2 as H2'. 2: exact H.
      destruct H2' as [[? ?] | [t [? [? [n4 [D1 ?]]]]]]; subst.
      + congruence.
      + epose proof (delay_internal_same _ _ _ H _ _ D1 _ H2).
        epose proof (IHD H6 _ _ _ _ H0 H1 D1).
        eapply internals_trans. 2: exact H4.
        exists [(AInternal, ι)], 1. split. repeat constructor.
        eapply ntrans. exact H3. apply nrefl.
    - apply not_eq_sym in n0.
      epose proof (D2 := step_chain _ _ _ _ H _ _ _ _ H2 n0).
      destruct D2 as [n4 D2].
      epose proof (IHD H6 _ _ _ _ H0 H1 D2).

      eapply internals_trans. 2: exact H3.
      exists [(AInternal, ι)], 1. split. repeat constructor.
      eapply ntrans. 2: apply nrefl.
      clear IHD H6 D All H3 H1.
      now epose proof (diamond_derivations _ _ _ _ _ _ _ n0 H H2 D2).
  Unshelve.
...
Proof.
  split; intros.
  * unfold onlyOne.
    eapply chain_to_end in H1 as H1'. 2: exact H0. destruct H1'; subst.
    - destruct H2 as [n3 [n4 [D1 [D2 D3]]]].
      congruence.
    - destruct H2 as [n3 D].
      exists n3. split.
      + exists q, n3. split. 2: split. 1,3: apply internals_refl. auto.
      + 
        eapply chain_to_front_iff in H0. 3: exact D. 2: auto. 2: exact H1.
        auto.
  * unfold onlyOne.
